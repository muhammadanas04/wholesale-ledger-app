...
import { Plus, Receipt, Trash2, Download, Calendar, Search, Tag, Edit2 } from 'lucide-react'
...
export default function OtherExpenses() {
  ...
  async function handleSaveCategory(e) {
    ...
    if (editingCategory) {
      try {
        await ipc('expense-categories:update', { id: editingCategory.id, name: newCategoryName.trim() })
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const headers = ["ID", "Date", "Category", "Reason", "Money Spent (₹)", "Money Gained (₹)"]
              ...
              <table className="w-full text-sm relative">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading ? (
                    ...
                    <>
                      ...
                      {expenses.length === 0 && (
                        <tr><td colSpan={6} className="text-center py-12 text-gray-400 italic">No entries recorded</td></tr>
                      )}
                    </>
                  )}
                </tbody>
                {!loading && expenses.length > 0 && (
                  <tfoot className="bg-gray-50 border-t-2 border-gray-200 text-xs font-bold text-gray-700">
                    <tr>
                      <td className="px-5 py-3 text-gray-900 font-black uppercase tracking-wider" colSpan={3}>
                        Total
                      </td>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm sticky top-6">
            <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between bg-gray-50/50">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-gray-800 text-sm uppercase tracking-wider">Categories</h2>
              </div>
            </div>
            ...
            <div className="overflow-auto max-h-[calc(100vh-250px)]">
              <ul className="divide-y divide-gray-100">
                {categories.length === 0 ? (
                  <li className="p-6 text-center text-sm text-gray-400 italic">No categories created yet</li>
                ) : (
                  categories.map(cat => (
                    <li key={cat.id} className="flex items-center justify-between p-3 hover:bg-gray-50 group">
